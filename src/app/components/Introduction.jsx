...
const mediaIconContainer = 'flex justify-center gap-8 xs:gap-10 md:gap-16 py-3 text-gray-600 text-3xl xs:text-4xl md:text-5xl dark:text-gray-400'
...
export default function Introduction({contactFormRef}) {
  return (
    <header>
      ...
      <div id='media-icon-container' className={mediaIconContainer}>        
        <a id='media-icon' className={mediaIcon} href={links[0]} target='_blank'><BsLinkedin/></a>
        <a id='media-icon' className={mediaIcon} href={links[1]} target='_blank'><BsGithub/></a>
        <a id='media-icon' className={mediaIcon} href={BASE_PATH + links[2]} target='_self'><BsBriefcaseFill/></a>
        <button id='media-icon' onClick={() => contactFormRef.current.scrollIntoView({behavior: 'smooth'})} className={mediaIcon}><BsMailbox2/></button>
      </div>
